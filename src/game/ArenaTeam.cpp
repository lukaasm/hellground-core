...
{
    if (!objmgr.GetPlayer(captainGuid.GetRawValue()))                 // player not exist
        return false;
    ...
    sLog.outArena("New ArenaTeam created [Id: %u] [Type: %u] [Captain GUID: %u]", GetId(), GetType(), GetCaptainGuid().GetRawValue());
...
bool ArenaTeam::AddMember(ObjectGuid playerGuid)
{
    ...
    Player *pl = objmgr.GetPlayer(playerGuid.GetRawValue());
    if (pl)
    {
    ...
    {
        //                                                     0     1
        QueryResult_AutoPtr result = CharacterDatabase.PQuery("SELECT name, class FROM characters WHERE guid='%u'", playerGuid.GetCounter());
        if (!result)
            return false;
        ...
        if (Player::GetArenaTeamIdFromDB(playerGuid.GetRawValue(), GetType()) != 0)
        {
    ...
    Player::RemovePetitionsAndSigns(playerGuid.GetRawValue(), GetType());
...
bool ArenaTeam::LoadArenaTeamFromDB(QueryResult_AutoPtr arenaTeamDataResult)
{
    ...
    m_CaptainGuid.Set(fields[2].GetUInt32());
...
bool ArenaTeam::LoadMembersFromDB(QueryResult_AutoPtr arenaTeamMembersResult)
{
    ...
    do
    {
        ...
        ArenaTeamMember newmember;
        newmember.guid.Set(fields[1].GetUInt32());
...
void ArenaTeam::SetCaptain(ObjectGuid guid)
{
    ...
    Player *oldcaptain = objmgr.GetPlayer(GetCaptainGuid().GetRawValue());
    if (oldcaptain)
    ...
    if (Player *newcaptain = objmgr.GetPlayer(guid.GetRawValue()))
    {
...
void ArenaTeam::DelMember(ObjectGuid guid)
{
    ...
    if (Player *player = objmgr.GetPlayer(guid.GetRawValue()))
    {
...
void ArenaTeam::Roster(WorldSession *session)
{
    ...
    {
        pl = objmgr.GetPlayer(itr->guid.GetRawValue());
...
void ArenaTeam::NotifyStatsChanged()
{
    ...
    for(MemberList::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        Player * plr = objmgr.GetPlayer(itr->guid.GetRawValue());
        if(plr)
...
void ArenaTeam::BroadcastPacket(WorldPacket *packet)
{
    for (MemberList::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        Player *player = objmgr.GetPlayer(itr->guid.GetRawValue());
        if(player)
...
bool ArenaTeam::IsFighting() const
{
    for (MemberList::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        if (Player *p = objmgr.GetPlayer(itr->guid.GetRawValue()))
        {
